...
namespace ShadowBot.Common.Utilities
{
    ...
    {
        private static ConcurrentDictionary<KeyboardHook, HHOOK> s_allHookInfos = new ConcurrentDictionary<KeyboardHook, HHOOK>();
        public static List<KeyboardHook> UninstallAll()
        {
            var UninstalledAll = new List<KeyboardHook>();
        ...
        public static void ReinstallAll(List<KeyboardHook> UninstalledAll)
        {
        ...
        public delegate void KeyboardHookCallback(KeyboardHookEventArgs args);
        ...
        private unsafe HHOOK SetHook(HOOKPROC proc)
        {
            using (ProcessModule module = Process.GetCurrentProcess().MainModule)
            {
                fixed (char* lpModuleNameLocal = module.ModuleName)
                {
                    ...
                    return PInvoke.SetWindowsHookEx(WINDOWS_HOOK_ID.WH_KEYBOARD_LL, proc, hmod, 0);
                }
            }
        }
        ...
        private LRESULT HookFunc(int nCode, WPARAM wParam, LPARAM lParam)
        {
            if (nCode >= 0)
            {
                uint iwParam = (uint)(nuint)wParam;
                var keyboardStruct = (KBDLLHOOKSTRUCT)Marshal.PtrToStructure(lParam, typeof(KBDLLHOOKSTRUCT));
                var kind = iwParam == PInvoke.WM_KEYDOWN || iwParam == PInvoke.WM_SYSKEYDOWN ? KeyState.KeyDown : KeyState.KeyUp;
                var args = new KeyboardHookEventArgs(kind, keyboardStruct);
                if (kind == KeyState.KeyDown)
                    KeyDown?.Invoke(args);
                if (kind == KeyState.KeyUp)
                    KeyUp?.Invoke(args);
                if (args.Handled)
    ...
    {
        public KeyboardHookEventArgs(KeyState kind, KBDLLHOOKSTRUCT keyboardStruct)
        {
            KeyState = kind;
            KeyboardStruct = keyboardStruct;
        }

        public KBDLLHOOKSTRUCT KeyboardStruct { get; private set; }

        public VKeys Key => (VKeys)KeyboardStruct.vkCode;
    ...
    {
        public static bool IsControl(this VKeys key)
        {
            return key == VKeys.LCONTROL || key == VKeys.CONTROL || key == VKeys.RCONTROL;
        }

        public static bool IsShift(this VKeys key)
        {
            return key == VKeys.LSHIFT || key == VKeys.SHIFT || key == VKeys.RSHIFT;
        }

        public static bool IsWin(this VKeys key)
        {
            return key == VKeys.LWIN || key == VKeys.RWIN;
        }

        public static bool IsAlt(this VKeys key)
        {
            return key == VKeys.LMENU || key == VKeys.MENU || key == VKeys.RMENU;
        }

        public static bool IsFunctional(this VKeys key)
        {
            return key == VKeys.ESCAPE
                || (key >= VKeys.F1 && key <= VKeys.F24)
                || key == VKeys.PRINT
                || key == VKeys.INSERT
                || key == VKeys.DELETE;
        }

        public static bool IsCharacter(this VKeys key)
        {
            return (key >= VKeys.KEY_0 && key <= VKeys.KEY_Z)
                  || (key >= VKeys.NUMPAD0 && key <= VKeys.DIVIDE)
                  || key == VKeys.SPACE;
        }

        public static bool IsOEM(this VKeys key)
        {
            return key >= VKeys.OEM_1 && key <= VKeys.OEM_8;
        }

        public static bool IsModifier(this VKeys key)
        {
            return key.IsAlt() || key.IsControl() || key.IsWin() || key.IsShift();
        }
    }
}
