...
namespace ShadowBot.Common.Utilities
{
    public static class WpfHelper
    {
        ...
        {
            window = MediaTypeNames.Application.Current.Windows.OfType<Window>().FirstOrDefault(predicate);
            return window != null;
        }
        ...
        {
            var windows = MediaTypeNames.Application.Current.Windows.OfType<Window>().Where(it => it.IsModal()).ToArray();
            return windows;
        }
        ...
        public static async Task CloseWindowAsync(this Window window, DispatcherPriority priority = DispatcherPriority.Normal)
        {
            ...
            await MediaTypeNames.Application.Current.Dispatcher.InvokeAsync(() =>
            {
                window.Close();
            }, priority);
        }
        ...
        {
            activeWindow = null;
            MediaTypeNames.Application current = MediaTypeNames.Application.Current;
            if (current == null)
            {
                return false;
            }
        ...
        {
            var streamInfo = MediaTypeNames.Application.GetResourceStream(uri);
            if (streamInfo == null)
    ...
    public static class WpfImageExtension
    {
        public static Task LoadInternetImageSourceAsync(this Image image, Uri uri)
        {
            return Task.Run(() =>
            {
                byte[] imageData = null;
                try
                {
                    using (var webClient = new TimeoutWebClient(5000))
                    {
                ...
                image.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    {
                        // 有些网址打开会导致内浏览器关闭  COMException
                        // FIX ISSUE-1009，下载下来的不是ico，而是html  NotSupportedException
                        return;
                    }
                    bitmapImage.Freeze();
        ...
        public static Icon UriToIcon(Uri uri)
        {
            if (uri.IsFile)
            {
                return new Icon(uri.LocalPath);
            }
            else if (uri.Scheme == "pack")
            {
                var resourceStream = LoadApplicationResourceStream(uri);
                return new Icon(resourceStream);
            }
            throw new NotImplementedException();
        }

        public static bool IsIconValid(string iconPath)
        {
            try
            {
    ...
    public class WinFormWindow : System.Windows.Forms.IWin32Window
    {
        ...
        public WinFormWindow(IntPtr hWnd)
        {
            _hWnd = hWnd;
        }
